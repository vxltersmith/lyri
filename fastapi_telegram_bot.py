...
def is_youtube_url(url):
    youtube_regex = (
        ...
        r"(watch\?v=|embed/|v/|shorts/|playlist\?|track/|.+\?v=)?([^&=%\?]{11})"
    )
...
async def handle_youtube(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        "Processing YouTube video... (This might take a while)"
    )
    ...
    try:
        ...
        await asyncio.get_event_loop().run_in_executor(
            None, download_youtube_video, message_text, output_path
        )
        ...
        logger.error(
            f"Error downloading YouTube video, you can try again later: {str(e)}"
        )
        ...
        inline_keyboard = [
            ...
            [
                InlineKeyboardButton(
                    "Drop current context / Начать сначала",
                    callback_data="drop_context",
                )
            ],
        ]
        ...
        await update.message.reply_text(
            f"Failed to download YouTube video: {str(e)}", reply_markup=markup
        )
    ...
    if image_file.get("is_sticker", False):
        ...
        try:
            ...
            try:
                ...
                raise Exception(
                    "Neither PIL with WebP support nor Wand is available. "
...
async def align(
    update: Update, context: ContextTypes.DEFAULT_TYPE, production_type: str
) -> None:
    logger = logging.getLogger(__name__)
    config = context.bot_data["config"]
    ...
    try:
        ...
        if background_file and not background_file.get("is_video_file"):
            ...
            try:
                ...
                await update.effective_chat.send_message(
                    f"An error occurred: {str(e)} \n Currently Telegram does not "
                    "support downloading files larger than 20Mb in bot API. "
        ...
        result_paths = await api_client.align(
            audio_file_name, data, background_file_path
        )
        ...
        if video_path:
            ...
            with open(video_path, "rb") as video:
                await update.effective_chat.send_video(
                    video, caption="Here's your video with aligned lyrics!"
                )
        if subtitles_path:
        ...
        if instrumental_audio_full_path:
            ...
            await update.effective_chat.send_message(
                "Sending instrumental audio file..."
            )
...
def setup_core_logic(bot_data):
    ...
    api_client = VideoAlignerClient(
        api_url=api_host,
        input_chache=bot_input_cache_path,
        output_cache_path=bot_cache_path,
    )
...
async def try_start_processing(
    ...
    if not is_context_full(user_data):
        ...
        if has_audio or has_video:
            inline_keyboard.append(
                [
                    InlineKeyboardButton(
                        "Separate audio / Разделить на дорожки",
                        callback_data="separate_audio",
                    )
                ]
            )
            inline_keyboard.append(
                [
                    InlineKeyboardButton(
                        "Automatic subtitles(premium)", callback_data="auto_subs"
                    )
                ]
            )

        if not has_lyrics and (
            has_audio
            and not user_data["audio_file"].get("is_youtube", False)
            and user_data["audio_file"]["title"]
            and user_data["audio_file"]["performer"]
        ):
            inline_keyboard.append(
                [
                    InlineKeyboardButton(
                        "Search for lyrics / Найти текст online (expirimental)",
                        callback_data="search_lyrics",
                    )
                ]
            )
    ...
    # await update.effective_chat.send_message(
    #     "Good! Now I'm settin up my core logic..."
    # )

    # Ask for video orientation
    if "aspect_ratio" not in user_data:
        await update.effective_chat.send_message(
            "Wonderful, we're ready, please choose video format"
        )
        inline_keyboard = [
            [
                InlineKeyboardButton("Vertical Video", callback_data="vertical_video"),
                InlineKeyboardButton(
                    "Horizontal Video", callback_data="horizontal_video"
                ),
            ],
            [
                InlineKeyboardButton(
                    "Drop context / Начать сначала", callback_data="drop_context"
                )
            ],
        ]
        markup = InlineKeyboardMarkup(inline_keyboard)
        await update.effective_chat.send_message(
            "Choose the video orientation:", reply_markup=markup
        )
    else:
        # If context is valid
        ...
        markup = InlineKeyboardMarkup(inline_keyboard)
        await update.effective_chat.send_message(
            "Choose an option below to get started:", reply_markup=markup
        )
...
async def handle_audio(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not audio_type:
        await update.message.reply_text(
            "Sorry but I don't think this is audio, please try another file"
        )
        return
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if update.message.document:
        ...
        if document and document.file_name.endswith(".txt"):
            ...
            await update.message.reply_text(
                "Lyrics file received! Use /align to start the alignment process."
            )
    else:
...
async def handle_background_image(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
...
async def handle_callback_query(
    ...
    if query.data == "music_align":
        await query.edit_message_text(
            "You selected: Music video 🎥. Processing will begin soon!"
        )
        await align(update, context, "music")
    elif query.data == "karaoke_align":
        await query.edit_message_text(
            "You selected: Karaoke video 🎤. Processing will begin soon!"
        )
        await align(update, context, "karaoke")
    elif query.data == "auto_subs":
        context.user_data["lyrics_file"] = None
        await query.edit_message_text(
            "You selected: Automatic subtitles, "
            "this is a heavy feature, please be patient"
        )
        await try_start_processing(update, context)
        ...
        await query.edit_message_text(
            "You selected: Separate audio 🎤. Processing will begin soon!"
        )
        # setup_core_logic(context)
        await align(update, context, "separate_audio")
    elif query.data == "cancel":
        await query.edit_message_text(
            "Processing cancelled. You can start a new session now. 😊"
        )
...
def main():
    ...
    app.add_handler(
        MessageHandler((filters.Document.TEXT | filters.TEXT), handle_document)
    )
