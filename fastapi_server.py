...
class AlignerServer:
    ...
    def setup_routes(self):
        ...
        @self.app.get("/")
        async def root():
            return {"message": "Aligner Server is running. Use /upload to upload a file."}
        ...
        async def start_processing(task_id: str, background_tasks: BackgroundTasks):
            ...
            if task_info["status"] != "Uploaded":
                return JSONResponse(status_code=400, content={"message": "File is not in 'Uploaded' status"})
        ...
        async def receive_metadata(task_id: str, request: Request):
            ...
            return JSONResponse(content={"status": "success", "received_data": json_data})
        
        @self.app.post("/upload/")
        async def upload_files(task_id: str = Form(None), files: list[UploadFile] = File(...), keys: list[str] = Form(...)):
            ...
            if len(files) != len(keys):
                return JSONResponse(status_code=400, content={"message": "Number of files and keys must match"})
            ...
            for file, key in zip(files, keys):
                ...
                logger.info(f"File uploaded: {input_file_path} (Task ID: {task_id}, Key: {key})")
    ...
    def process_file(self, task_id: str):
        ...
        try:
            ...
            logger.info(f"Processing started for {input_file_path} (Task ID: {task_id})")
            ...
            if instrumental_audio_full_path:
                ...
                logger.info(f"Artifact: {instrumental_audio_full_path} (Task ID: {task_id})")
            if input_audio_path:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./configs/server_default.yaml", help="Path to the configuration file")
    args = parser.parse_args()
    
    config_dict = load_settings(args.config)
    config = Config(config_dict.get('paths', {}).get('input_cache', "input_cache"), 
